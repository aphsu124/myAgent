...
def main():
    ...
    while attempts < max_attempts and not is_valid:
        ...
        請根據新聞來源撰寫一份專業報告。
        1. 針對新聞事件給出對「我們油廠」的實質影響與具體建議。
        2. 嚴禁包含簽名落款。
        3. 嚴禁包含與泰國棕櫚油無關之內容（包含但不限於洋蔥等其他農產品）。
        4. 預設受眾為高層主管，嚴禁解釋「FFB、CPO」等基本名詞定義，必須將篇幅完全投入於「對獲利、庫存與產能的深度策略建議」，確保內容豐富且具備專業深度。
        5. 最後一行必須輸出：DATA_JSON: {{\"ffb\": 8.1, \"cpo\": 45.5, \"bmd_myr\": 4828, \"ex_rate\": 7.78}}
